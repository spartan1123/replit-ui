          ...
          <Card className="bg-card/40 border-border/50 p-6 backdrop-blur-md">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-orange-400">
                ...
                <h3 className="font-semibold text-sm uppercase tracking-wide">Rapid Fire</h3>
              </div>
              <Badge className="text-[10px] bg-orange-500/10 text-orange-400 border-orange-500/20">1.8s</Badge>
            </div>
            <p className="text-xs text-muted-foreground mb-4">Compact semi-auto clicks into delete, humanize turn on click.</p>

            <div className="space-y-4">
              <SettingToggle
                label="Rapid Fire Multiplier"
                description="Mode 0 - Saber 0.0"
                enabled={false}
                onChange={() => {}}
              />
              ...
              <SliderControl
                ...
                value={tuning.rapidFire.fireRate}
                onChange={(v) => setTuning({ ...tuning, rapidFire: { ...tuning.rapidFire, fireRate: v } })}
                min={0}
                max={10}
              />
              ...
              <SliderControl
                ...
                value={tuning.rapidFire.humanizationLevel}
                onChange={(v) => setTuning({ ...tuning, rapidFire: { ...tuning.rapidFire, humanizationLevel: v } })}
                min={0}
                max={10}
              />

              <SliderControl
                label="Jitter Probability"
                value={tuning.rapidFire.jitterProbability}
                onChange={(v) => setTuning({ ...tuning, rapidFire: { ...tuning.rapidFire, jitterProbability: v } })}
                min={0}
                max={1}
              />
              ...
              <SliderControl
                ...
                value={tuning.rapidFire.heatSimulation}
                onChange={(v) => setTuning({ ...tuning, rapidFire: { ...tuning.rapidFire, heatSimulation: v } })}
                min={0}
                max={1}
              />
              ...
              <SettingToggle
                label="Burst Mode"
                description="Fix in controlled bursts instead of continuous fire"
                enabled={false}
                onChange={() => {}}
              />
              ...
              <SettingToggle
                ...
                description="Adjust fire rate based on environmental conditions"
                enabled={true}
                onChange={() => {}}
              />
              ...
              <SettingToggle
                ...
                description="Keeps click timing inside neutral windows"
                enabled={true}
                onChange={() => {}}
          ...
          <Card className="bg-card/40 border-border/50 p-6 backdrop-blur-md">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2 text-cyan-400">
                ...
                <h3 className="font-semibold text-sm uppercase tracking-wide">Humanization & Movement</h3>
              </div>
            </div>
            <p className="text-xs text-muted-foreground mb-4">Add realistic imperfections and smooth movement enhancements.</p>

            <div className="space-y-4">
              <div className="space-y-2 p-3 bg-black/20 rounded-lg border border-white/5">
                <p className="text-xs text-muted-foreground">Variance 0.00 | Fatigue 0.0%</p>
              </div>

              <SliderControl
                label="Variance Level"
                value={tuning.humanization.varianceLevel}
                onChange={(v) => setTuning({ ...tuning, humanization: { ...tuning.humanization, varianceLevel: v } })}
                min={0}
                max={5}
                displayValue={tuning.humanization.varianceLevel.toFixed(2)}
              />

              <SliderControl
                label="Error Rate"
                value={tuning.humanization.errorRate}
                onChange={(v) => setTuning({ ...tuning, humanization: { ...tuning.humanization, errorRate: v } })}
                min={0}
                max={1}
                displayValue={(tuning.humanization.errorRate * 100).toFixed(1) + "%"}
              />

              <SliderControl
                label="Reaction Time Variation"
                value={tuning.humanization.reactionTime}
                onChange={(v) => setTuning({ ...tuning, humanization: { ...tuning.humanization, reactionTime: v } })}
                min={0}
                max={1}
                displayValue={tuning.humanization.reactionTime.toFixed(2)}
              />

              <SliderControl
                label="Input Noise Level"
                value={tuning.humanization.inputNoiseLevel}
                onChange={(v) => setTuning({ ...tuning, humanization: { ...tuning.humanization, inputNoiseLevel: v } })}
                min={0}
                max={1}
                displayValue={(tuning.humanization.inputNoiseLevel * 100).toFixed(1) + "%"}
              />

              <SettingToggle
                label="Pattern Learning"
                description="AI learns and applies contextual enhancements"
                enabled={true}
                onChange={() => {}}
              />

              <SettingToggle
                label="Fatiguation Simulation"
                description="Gradually degrade performance over time for realism"
                enabled={true}
                onChange={() => {}}
              />
              ...
              <SettingToggle
                ...
                description="Apply all enhancement systems"
                enabled={true}
                onChange={() => {}}
                activeStatus="System Active"
              />
